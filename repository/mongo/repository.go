...
import (
	...
	"github.com/martinitorquato/go-shortener"
...
func (r *mongoRepository) Find(code string) (*shortener.RedirectRepository, error) {
	ctx, cancel := context.WithTimeout(context.Background(), r.timeout)
	defer cancel()
	redirect := &shortener.RedirectRepository{}
...
func (r *mongoRepository) Store(redirect *shortener.RedirectRepository) error {
	ctx, cancel := context.WithTimeout(context.Background(), r.timeout)
	defer cancel()
	collection := r.client.DataBase(r.database).Collection("redirects")
